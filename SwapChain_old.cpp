...
SwapChain_old::SwapChain_old(Window& window, Device& device, SyncObjects& syncObjects ):
                      window( window ), device( device ), syncObjects( syncObjects ) {
    createSwapChain();
    createImageViews();
}

SwapChain_old::~SwapChain_old() {
...
VkExtent2D SwapChain_old::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
void SwapChain_old::createSwapChain() {
...
void SwapChain_old::createImageViews() {
...
VkResult SwapChain_old::acquireNextImage(uint32_t *imageIndex ) {
    auto logicalDevice = device.getLogicalDevice();
...
void SwapChain_old::updateCurrentFrame() {
